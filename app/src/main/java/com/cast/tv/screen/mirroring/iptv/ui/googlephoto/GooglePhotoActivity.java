package com.cast.tv.screen.mirroring.iptv.ui.googlephoto;
...
import com.cast.tv.screen.mirroring.iptv.databinding.ActivityGooglePhotoBinding;
...
public class GooglePhotoActivity extends BaseBindingActivity<ActivityGooglePhotoBinding, GooglePhotoViewModel> implements GooglePhotoNavigator {

    private GooglePhotoViewModel mGooglePhotoViewModel;
    private ActivityGooglePhotoBinding mActivityGooglePhotoBinding;
    ...
    @Override
    public GooglePhotoViewModel getViewModel() {
        mGooglePhotoViewModel = ViewModelProviders.of(this).get(GooglePhotoViewModel.class);
        return mGooglePhotoViewModel;
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mGooglePhotoViewModel.setNavigator(this);
        mActivityGooglePhotoBinding = getViewDataBinding();
    ...
    protected void initView() {
        ...
        preloadGooglePhotoAdsIfInit();
        Admod.getInstance().loadSmallNativeFragment(this, BuildConfig.native_click_item_local_data_id, mActivityGooglePhotoBinding.nativeAds);
    }
    ...
    private void setupChromecastConnection() {
        mDefaultCastStateListener.setCastIcon(mActivityGooglePhotoBinding.castImg);
        ...
        mActivityGooglePhotoBinding.castImg.setOnClickListener(view -> {
            FirebaseUtils.sendEventFunctionUsed(this, FirebaseUtils.CAST_BUTTON_EVENT, "Click cast button", "Google Photo layout");
    ...
    private void setForClick() {
        mActivityGooglePhotoBinding.backImg.setOnClickListener(view -> onBackPressed());
        mActivityGooglePhotoBinding.accessButtonArea.setOnClickListener(view -> {
            FirebaseUtils.sendEventFunctionUsed(this, FirebaseUtils.GOOGLE_PHOTO_EVENT, "Click go to Google Photo");
            startSelectFileFromGooglePhoto();
        });
    }
    ...
    private void showPermissionIssueArea() {
        mActivityGooglePhotoBinding.noPermissionArea.setOnClickListener(v -> {
            startRequestPermission();
        });
        mActivityGooglePhotoBinding.noPermissionArea.setVisibility(View.VISIBLE);
        mActivityGooglePhotoBinding.accessButtonArea.setVisibility(View.GONE);
    }

    private void showAccessButtonArea() {
        mActivityGooglePhotoBinding.noPermissionArea.setVisibility(View.GONE);
        mActivityGooglePhotoBinding.accessButtonArea.setVisibility(View.VISIBLE);
    }

    private void startSelectFileFromGooglePhoto() {
        if (!PackageUtils.isAppAvailable(this, AppConstants.GOOGLE_PHOTO_PACKAGE_NAME)) {
            ...
            packageNotFound.setConfirmButton(getString(R.string.confirm_text), sweetAlertDialog -> {
                sweetAlertDialog.dismiss();
                PackageUtils.gotoDownloadApp(this, AppConstants.GOOGLE_PHOTO_PACKAGE_NAME);
            });
        ...
        intent.setPackage(AppConstants.GOOGLE_PHOTO_PACKAGE_NAME);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        showAdsBeforeAction(mGooglePhotoInterstitialAd, () -> {
            try {
                startActivityForResult(Intent.createChooser(intent, String.valueOf(R.string.select_file_title)), TAKE_FILE_FROM_GOOGLE_PHOTO_REQUEST);
                mIsOpening = true;
    ...
    protected void updateFilePathFromCloudServer(Uri uri, String filePath) {
        ...
        if (filePath != null && FileUtils.checkSupportedFile(filePath)) {
            ...
            if (FileUtils.isVideoFile(filePath)) {
                ...
                SweetAlertDialog errorLoadFile = DialogFactory.getDialogError(this, getString(R.string.activity_google_photo_not_valid_file));
                errorLoadFile.show();
    ...
    private void startPlayerConnection(FileData fileData) {
        ...
        if (fileData.getFileType().equals(DataConstants.FILE_TYPE_VIDEO)) {
            intent = new Intent(GooglePhotoActivity.this, PlayerActivity.class);
        } else {
            intent = new Intent(GooglePhotoActivity.this, ViewerActivity.class);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == TAKE_FILE_FROM_GOOGLE_PHOTO_REQUEST) {
            mIsOpening = false;
            ...
            if (RealPathUtil.getInstance().isGooglePhotoFile(uri)) {
            ...
            public void onStatusUpdated() {
                if (mSelectedMedia != null && mSelectedMedia.getFileType() != null && !mSelectedMedia.getFileType().equals(DataConstants.FILE_TYPE_PHOTO)) {
                    if (remoteMediaClient.isPlaying() || remoteMediaClient.isPaused() || remoteMediaClient.isBuffering()) {
                        ...
                        Intent intent = new Intent(GooglePhotoActivity.this, ExpandedControlsActivity.class);
                        startActivity(intent);
