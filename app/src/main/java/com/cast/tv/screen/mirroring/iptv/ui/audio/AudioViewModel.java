...
public class AudioViewModel extends BaseViewModel<AudioNavigator> {
    ...
    public void getFileList(int order) {
        AsyncTask.execute(() -> {
            ArrayList<FileData> allData = FileUtils.getAllExternalFileList(getApplication(), DataConstants.FILE_TYPE_AUDIO, order);
            if (allData != null && allData.size() > 0) {
                getCompositeDisposable().add(getDataManager()
                        .getListBookmarkByType(DataConstants.FILE_TYPE_AUDIO)
